...
Substantially, the `from.Vprop` score thus indicates that 83.12% of political news messages in *online NP 1* is similar or identical to recent *newsagency* messages.
...
This can help present the document similarity data in an intuitively interpretable way.
For the sake of convenience, we offer `plot.aggregate.network` function.
...
For this example we use the `from.Vprop` edge attribute with a threshold of 0.2.

```{r, fig.align='center', fig.width=7, fig.height=5}
plot.aggregate.network(g.agg, weight.var = 'from.Vprop',
                       weight.thres = 0.2)
```
...
plot.aggregate.network(g2.agg, weight.var = 'from.Vprop',
                       weight.thres = 0.2)
```
...
g_subcomps = decompose.graph(g)
plot.document.network(g_subcomps[[1]], dtm=dtm)

# Aggregate network 
g.agg = aggregate.network(g, by='source', edge.attribute='hourdiff', agg.FUN=median)

get.adjacency(g.agg, attr='from.Vprop')
plot.aggregate.network(g.agg, weight.var = 'from.Vprop', weight.thres=0.2)
```
