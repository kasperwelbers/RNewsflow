...
To make this more convenient, we also offer the `plot.directed.network` function. 
...
Here we also demonstrate the `return.df` parameter in the `aggregate.network` function.
...
plot.directed.network(g.agg, weight.var = 'to.Vprop', weight.thres=0.2)

g2 = only.first.match(g)
g2.agg = aggregate.network(g2, by='source', edge.attribute='hourdiff', agg.FUN=median)

get.adjacency(g2.agg, attr='to.Vprop')
plot.directed.network(g2.agg, weight.var = 'to.Vprop', weight.thres=0.2)
