...
We therefore offer the \emph{documents.window.compare} function which compares documents using a sliding window over time.
...
The second part shows how the core function of this packages, \emph{documents.window.compare}, is used to calculate document similarities for many documents over time.
...
Given a DTM and corresponding document meta data, the document similarities over time can be calculated with the \emph{documents.window.compare} function.
...
A vertex.attribute can also be given to view the windows grouped by this attribute.
...
To make this more convenient, we also offer the \emph{plot.aggregate.network} function. 
