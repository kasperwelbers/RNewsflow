...
The data is based on a recent study on the influence of a Dutch news agency on the print and online editions of Dutch newspapers in political news coverage [@welbers18].
...
To calculate this, we provide the `term.day.dist` function. 
...
Although the demonstration in this vignette used a moderate sized dataset, the `newsflow.compare` function can handle much larger data and is fast, thanks to a dedicated Rcpp (c++) implementation of sparse matrix multiplication for comparing documents within the given date window.
