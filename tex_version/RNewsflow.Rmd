...
We offer the `RNewsflow` R package[^reviewernote] as a toolkit to analyze the homogeneity and diffusion of news content using computational text analysis.
...
We therefore offer the `newsflow.compare` function which compares documents using a sliding window over time.
...
The second part shows how the core function of this packages, `newsflow.compare`, is used to calculate document similarities for many documents over time.
...
To use lemmatization or part-of-speech tagging there are several free to use grammar parsers, such as *CoreNLP* for English [@corenlp] and *Frog* for Dutch [@bosch07][^external].

[^external]: To create a DTM based on externally pre-processed documents, one can use the base function `xtabs` to create a sparse matrix and the `as.DocumentTermMatrix` function from the `tm` package to transform this matrix to the `DocumentTermMatrix` class.
...
Given a DTM and corresponding document meta data, the document similarities over time can be calculated with the `newsflow.compare` function.
...
The `hour.window` parameter determines the time window in hours within which each document is compared to other documents. The argument is a vector of length 2, in which the first and second value determine the left and right side of the window, respectively. 
...
We specifify the `to.attribute` parameter as "source", and leave the `from.attribute` parameter empty. 
...
To visualize these components, we offer the `document.network.plot` function.
...
Here we demonstrate the `only.first.match` function, which transforms the network so that a document only has an edge to the earliest dated document it matches within the specified time window[^duplicate].
...
This package offers the  `network.aggregate` function as a versatile way to aggregate the edges of the document similarity network based on the vertex attributes (i.e. the document meta information).
...
Alternatively, `igraph` has the `get.adjacency` function to return the values for one edge attribute as an adjacency matrix, with the vertices in the rows and columns.
...
To make this more convenient, we also offer the `directed.network.plot` function. 
...
In the network.aggregate function it is possible to use different vertex attributes to aggregate the edges for `from` and `to` nodes.
...
Here we also demonstrate the `return.df` parameter in the `network.aggregate` function.
...
[^notfrom]: The *from.Vprop* cannot be interpreted in the same way, because it gives the proportion of all messages in the `from` group. If one is interested in the *from.Vprop* score per day, the *by.from* and *by.to* arguments in *network.aggregate* need to be switched. Note that one should not simply aggregate both *by.from* and *by.to* by date, because then only documents that both occured on this date will be aggregated. 

Another usefull application of this feature is to only aggregate the `by.to` group, by using the document name in the `by.from` argument. 
This way, the data in the *by.to* group is aggregated for each individual message. 
...
We set the `edge.attribute` to "weight" and `agg.FUN` to max, so that for each document we can see how strong the strongest match with each source was. 
...
The primary focus and most important feature of this package is the `newsflow.compare` function.
